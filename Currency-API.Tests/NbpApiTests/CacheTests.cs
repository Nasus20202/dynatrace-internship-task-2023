...
public class CacheTests
{
    ...
    public void Test1()
    {
        ...
        nbpApi.GetAverageExchangeRate("eur", date1).Wait();
        Assert.Equal(1, memoryCache.Count);
        nbpApi.GetAverageExchangeRate("eur", date2).Wait();
        Assert.Equal(2, memoryCache.Count);
        nbpApi.GetAverageExchangeRate("eur", date2).Wait();
        Assert.Equal(2, memoryCache.Count);
        nbpApi.GetAverageExchangeRate("eur", date3).Wait();
        Assert.Equal(3, memoryCache.Count);
        nbpApi.GetAverageExchangeRate("eur", date1).Wait();
        Assert.Equal(3, memoryCache.Count);
        nbpApi.GetMinAndMaxAverageExchangeRate("eur", 50).Wait();
